<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) in addresses" :key="address.id">
        <span class="username" :class="{selected: defaultAddress.id === address.id}">{{ address.consignee }}</span>
        <p @click="updateSelect(index)">
          <span class="s1">{{ address.userAddress }}</span>
          <span class="s2">{{ address.phoneNum }}</span>
          <span class="s3" v-show="defaultAddress.id === address.id">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="goods in tradeInfo.detailArrayList">
          <li>
...
export default {
  ...
  computed: {
    ...mapState({
      addresses: state => state.trade.addresses,
